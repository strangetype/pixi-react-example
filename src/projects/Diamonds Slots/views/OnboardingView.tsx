...
import { ONB_PROPS_MAP } from 'const/Diamonds Slots/ONB_MAP';
import { TextW } from 'components/Common/wrapper/TextW';
import { ONB_SKIP_BTN_TXT_STYLE, ONB_TXT_STYLE } from 'const/Diamonds Slots/textStyles';
...
import { machinePosition, panelPosition, skipPosition } from 'config/Diamonds Slots/positions';
...
export const OnboardingView = () => {
    ...
    useEffect(() => {
        const computed = useStoreComputed(useStorePopupsState, ['isShowOnboarding'], ({ isShowOnboarding, isKeyBoard }) => {
            setIsShowOnboarding(isShowOnboarding);
        });

        const computedGames = useStoreComputed(useStoreGamesState, ['onboardingSummaActive'], () => {
    ...
    const stateButton = useCallback((state) => {
    ...
    return (
        isShowOnboarding && !isResize && <>
            ...
            >
                ...
                >
                    <TextW
                        parentSlotName="text1"
                        style={new TextStyle(ONB_TXT_STYLE)}
                        anchor={0.5}
                        scale={[1, -1]}
                        text={translations['onboarding.step1']?.replace('$2', `${fractionDigits(limits.min_bet, 2, 2)} ${currency_code}`)}
                        props={{
                            [PORTRAIT]: ONB_PROPS_MAP[0] && ONB_PROPS_MAP[0].v ? ONB_PROPS_MAP[0].v : {},
                            [LANDSCAPE]: ONB_PROPS_MAP[0] && ONB_PROPS_MAP[0].h ? ONB_PROPS_MAP[0].h : {}
                        }}
                    />
                </SpineW>
                }

                {isShowAnimations[2] && <SpineW
                    spineDataKey="onboarding"
                    animations={[
                ...
                >
                    <TextW
                        ...
                        props={{
                            [PORTRAIT]: ONB_PROPS_MAP[1] && ONB_PROPS_MAP[1].v ? ONB_PROPS_MAP[1].v : {},
                            [LANDSCAPE]: ONB_PROPS_MAP[1] && ONB_PROPS_MAP[1].h ? ONB_PROPS_MAP[1].h : {}
                        }}
                    />
                    <TextW
                        parentSlotName="text2"
                        style={new TextStyle(ONB_TXT_STYLE)}
                        anchor={0.5}
                        scale={[1, -1]}
                        text={translations['onboarding.step3']}
                        props={{
                            [PORTRAIT]: ONB_PROPS_MAP[2] && ONB_PROPS_MAP[2].v ? ONB_PROPS_MAP[2].v : {},
                            [LANDSCAPE]: ONB_PROPS_MAP[2] && ONB_PROPS_MAP[2].h ? ONB_PROPS_MAP[2].h : {}
                        }}
                    />
                </SpineW>
                }

                {isShowAnimations[3] && <SpineW
                    spineDataKey="onboarding"
                    animations={[
                ...
                >
                    <TextW
                        ...
                        props={{
                            [PORTRAIT]: ONB_PROPS_MAP[3] && ONB_PROPS_MAP[3].v ? ONB_PROPS_MAP[3].v : {},
                            [LANDSCAPE]: ONB_PROPS_MAP[3] && ONB_PROPS_MAP[3].h ? ONB_PROPS_MAP[3].h : {}
                        }}
                    />
                </SpineW>
                }
            ...
            >
                <SpineW
                    spineDataKey="btn_skip"
                    animations={[
                        { configName: 'пропустить: появление', name: 'show1', loop: true },
                        { configName: 'пропустить: пост анимация', name: AnimationButton_F.normal, loop: true }
                    ]}
